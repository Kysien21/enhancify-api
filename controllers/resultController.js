...
exports.getresults = async (req, res) => {
  try {
    const results = await Result.find({ userId: req.session.user._id }).sort({ createdAt: -1 });
...
exports.getscore = async (req, res) => {
  try {
    ...
    if (!latestResult) return res.status(404).json({ message: "No resume score found" });
    res.json({
...
exports.getOriginalResume = async (req, res) => {
  try {
    const { resultId } = req.params;
    const result = await Result.findOne({ _id: resultId, userId: req.session.user._id });
    if (!result) return res.status(404).json({ message: "Original resume not found." });
...
exports.getOptimizedResume = async (req, res) => {
  try {
    ...
    const result = await Result.findOne({ _id: resultId, userId: req.session.user._id });
    if (!result) return res.status(404).json({ message: "Result not found." });
    if (!result.optimizedResume) return res.status(404).json({ message: "Resume not optimized yet." });
    res.json({ success: true, optimized: result.optimizedResume, isOptimized: result.isOptimized });
  } catch (err) {
...
exports.saveToHistory = async (req, res) => {
  try {
    ...
    if (!result.isOptimized || !result.optimizedResume) {
      return res.status(400).json({ message: "Please optimize the resume before saving to history" });
    }
    if (result.savedToHistory) return res.status(400).json({ message: "Already saved to history" });
    ...
    await result.save();
    res.json({ success: true, message: "✅ Optimized resume saved to history" });
  } catch (error) {
...
// Original PDF is NOT deleted here anymore
// It gets deleted when user clicks Download in the frontend
exports.downloadOptimized = async (req, res) => {
  try {
    ...
    if (!enhanced) {
      return res.status(400).json({ success: false, message: "Enhanced resume not found" });
    }

    const doc = new PDFDocument({ margin: 50, size: "LETTER" });

    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename=optimized-resume-${Date.now()}.pdf`);

    doc.pipe(res);

    // === CONTACT ===
    doc.fontSize(22).font("Helvetica-Bold").text(enhanced.contact.name, { align: "center" });
    doc.moveDown(0.3);
    const contactLine = [enhanced.contact.email, enhanced.contact.phone, enhanced.contact.location]
      .filter(Boolean).join(" | ");
    ...
    if (enhanced.contact.linkedin) {
      doc.fontSize(10).text(enhanced.contact.linkedin, {
        align: "center", link: enhanced.contact.linkedin, underline: true, color: "blue",
      });
    }
    doc.moveDown(1.5);
    ...
    if (enhanced.summary) {
      ...
      doc.moveDown(0.5);
      doc.fontSize(10).font("Helvetica").text(enhanced.summary, { align: "justify", lineGap: 2 });
      doc.moveDown(1.2);
    }

    // === EXPERIENCE ===
    if (enhanced.experience && enhanced.experience.length > 0) {
      doc.fontSize(14).font("Helvetica-Bold").text("WORK EXPERIENCE", { underline: true });
      doc.moveDown(0.5);
      enhanced.experience.forEach((exp, index) => {
        doc.fontSize(12).font("Helvetica-Bold").text(exp.position);
        doc.fontSize(10).font("Helvetica-Oblique").text(`${exp.company} | ${exp.period}`);
        doc.moveDown(0.3);
        exp.responsibilities.forEach((resp) => {
          doc.fontSize(10).font("Helvetica").text(`• ${resp}`, { indent: 20, align: "justify", lineGap: 1 });
          doc.moveDown(0.2);
        });
        if (index < enhanced.experience.length - 1) doc.moveDown(0.8);
      });
      doc.moveDown(1.2);
    }
    ...
    if (enhanced.education && enhanced.education.length > 0) {
      ...
      enhanced.education.forEach((edu) => {
        ...
        doc.fontSize(10).font("Helvetica").text(`${edu.institution} | ${edu.period}`);
        if (edu.relevant) {
          doc.fontSize(9).font("Helvetica-Oblique").text(`Relevant Coursework: ${edu.relevant}`, { indent: 20 });
        }
        doc.moveDown(0.6);
      });
      doc.moveDown(0.8);
    ...
    if (enhanced.skills) {
      ...
      if (enhanced.skills.technical && enhanced.skills.technical.length > 0) {
        ...
        doc.fontSize(10).font("Helvetica").text(enhanced.skills.technical.join(" • "), { indent: 20, lineGap: 1 });
        doc.moveDown(0.4);
      }
      if (enhanced.skills.soft && enhanced.skills.soft.length > 0) {
        doc.fontSize(11).font("Helvetica-Bold").text("Soft Skills:");
        doc.fontSize(10).font("Helvetica").text(enhanced.skills.soft.join(" • "), { indent: 20, lineGap: 1 });
        doc.moveDown(0.4);
      }
      doc.moveDown(0.8);
    }
    ...
    if (enhanced.certifications && enhanced.certifications.trim() !== "") {
      ...
      doc.moveDown(0.5);
      doc.fontSize(10).font("Helvetica").text(enhanced.certifications, { lineGap: 1 });
      doc.moveDown(0.8);
    }

    // === LANGUAGES ===
    if (enhanced.languages && enhanced.languages.length > 0) {
      doc.fontSize(14).font("Helvetica-Bold").text("LANGUAGES", { underline: true });
      doc.moveDown(0.5);
      doc.fontSize(10).font("Helvetica").text(enhanced.languages.join(" • "));
    }

    doc.end();
    console.log("✅ Optimized PDF generated and sent successfully");
